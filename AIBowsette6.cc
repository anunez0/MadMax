  ...
  void move_warriors() {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (can_move(id)) {
        ...
        else if (cell(unit(id).pos).type == City and (cell(unit(id).pos).owner != me() or allies_in_city(unit(id).pos) <= 3 or round() < 120)) command(id, d);
