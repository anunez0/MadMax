 ...
 */
#define PLAYER_NAME LastBowsette
...
struct PLAYER_NAME : public Player {
  ...
  Dir search_city(const Pos& p, Dir& d) {
    ...
    int min = rows();
    for (int i = 0; i < 8; ++i) {
      if ((cell(vc[i]).owner != me() or allies_in_city(vc[i]) < 3) and gc[i][p.i][p.j] < min) {
        i_min = i;
        min = gc[i][p.i][p.j];
      }
    }
  ...
  bool search_car_warrior(const Pos& p, Dir& d) {
    ...
    if (not pos_ok(p + d) or (cell(p + d).type != Desert and cell(p + d).type != Road and cell(p + d).type != City)) {
  ...
  bool search_car_car(const Pos& p, Dir& d) {
    ...
    while (not q.empty() and not b) {
      ...
      for (int i = 0; i < 8 and not b; ++i) {
        Pos s = r + Dir(i);
        if (pos_ok(s) and (cell(s).type == Desert or cell(s).type == Road) and not visited[s.i][s.j] and s.i <= (p + d).i + 1 and s.i >= (p + d).i - 1 and s.j <= (p + d).j + 1 and s.j >= (p + d).j - 1) {
          if (cell(s).id != -1 and unit(cell(s).id).type == Car and unit(cell(s).id).player != me()) {
            d = None;
            b = true;
          }
  ...
  Dir search_warrior_board(const Pos& p, Dir& d) {
    Pos pwar;
    vector<vector<bool> > visited(rows(), vector<bool>(cols(), false));
    visited[p.i][p.j] = true;
    queue<Pos> q;
  ...
  bool pos_ok_zone(const Pos& p, int z) {
    return (z == 0 and p.i < rows()/2 and p.j < cols()/2) or (z == 1 and p.i < rows()/2 and p.j >= cols()/2) or (z == 2 and p.i >= rows()/2 and p.j < cols()/2) or (z == 3 and p.i >= rows()/2 and p.j >= cols()/2);
  }
  ...
  Dir search_warrior(const Pos& p, Dir& d) {
    ...
    bool b2 = false;
    for (map<int, Pos>::const_iterator it = mp.begin(); it != mp.end() and not b2; ++it) {
      if (it->second == p) b2 = true;
      else if (unit(it->first).type == Car) ++z;
    }
    z %= 4;
    if (not pos_ok_zone(p, z)) return search_warrior_board(p, d);
    Pos pwar;
    vector<vector<bool> > visited(rows(), vector<bool>(cols(), false));
    visited[p.i][p.j] = true;
    queue<Pos> q;
    ...
    while (not q.empty() and not b) {
      ...
      for (int i = 0; i < 8 and not b; ++i) {
        Pos s = r + Dir(i);
        if (pos_ok(s) and (cell(s).type == Desert or cell(s).type == Road) and not visited[s.i][s.j] and pos_ok_zone(s, z)) {
          if (cell(s).id != -1 and unit(cell(s).id).type == Warrior and unit(cell(s).id).player != me()) {
            pwar = s;
            b = true;
          }
    ...
    if (b) {
      ...
      bfs_station(gwar, pwar);
      d = search(gwar, p, d);
    } else return search_warrior_board(p, d);
    return d;
  }
  ...
  void move_warriors() {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (can_move(id)) {
        Dir d = None;
        if (cell(unit(id).pos).type != City and search_car_warrior(unit(id).pos, d)) command(id, d);
        else if (unit(id).water <= gw[unit(id).pos.i][unit(id).pos.j] + 2*damage()) command(id, search(gw, unit(id).pos, d));
        else if (cell(unit(id).pos).type == City and (cell(unit(id).pos).owner != me() or allies_in_city(unit(id).pos) <= 3 or round() < 4*(warriors_health() - 2*damage()))) command(id, d);
  ...
  void move_cars() {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (can_move(id)) {
        Dir d = None;
        if (next_to_warrior(unit(id).pos, d)) {}
        else if (lost(unit(id).pos)) search(gr, unit(id).pos, d);
        else if (unit(id).food < gs[unit(id).pos.i][unit(id).pos.j] + 10) search(gs, unit(id).pos, d);
        else search_warrior(unit(id).pos, d);
        if (search_car_car(unit(id).pos, d)) {}
        command(id, d);
        mp[id] = unit(id).pos + d;
      }
    }
