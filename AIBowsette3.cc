...
struct PLAYER_NAME : public Player {
  ...
  bool search_car_car(const Pos& p, Dir& d) {
    ...
    if (pos_ok(p + d) and cell(p + d).type == Desert) {
    ...
    if (not pos_ok(p + d) or (cell(p + d).type != Desert and cell(p + d).type != Road)) {
  ...
  bool pos_ok_zone(const Pos& p, int z) {
    return (z == 0 and p.i < rows()/2 + 10 and p.j < cols()/2 + 10) or (z == 1 and p.i < rows()/2 + 10 and p.j >= cols()/2 - 10) or (z == 2 and p.i >= rows()/2 - 10 and p.j < cols()/2 + 10) or (z == 3 and p.i >= rows()/2 - 10 and p.j >= cols()/2 - 10);
  }
  ...
  void move_cars() {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (can_move(id)) {
        ...
        else if (unit(id).food < gs[unit(id).pos.i][unit(id).pos.j] + 10) command(id, search(gs, unit(id).pos, d));
        else if (next_to_warrior(unit(id).pos, d)) command(id, d);
